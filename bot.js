...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),
      ...
      botSOS = /^\/sos/; botScheduleForm = /^\/ScheduleForm/;
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("https://docs.google.com/document/d/1WoarNHEFTHRl410sORAdNsJPQXzm16JgrXEX7FXuLMk/edit?usp=sharing");
    this.res.end();
  } 
  else if(request.text && botScheduleForm.test(request.text)) {
    this.res.writeHead(200);
    postMessage("http://www.tinyurl.com/fallhoursform");
    this.res.end();
  } 
    ...
    this.res.writeHead(200);
    postMessage("current commands: /work /google /rtfm /inventory /sos /schedule /classchecks /directory /cool guy /faculty /shrug");
    this.res.end();
  }
